...
const coreEndpointDefinitions: EndpointDefinition[] = [
  ...
  },
  {
    ...
    method: "GET",
    path: "/graphql",
    summary: "GraphQL wrapper for the REST API.",
    description: "Query multiple resources in a single request with a GraphQL query. Supports divisions, districts, upazilas, unions, holidays, encyclopedia categories, and search.",
    cacheTtl: "1 minute",
    parameters: [],
    sampleResponse: { data: { divisions: [] } },
    recipes: ["Fetch related data in one request", "Reduce over-fetching with field selection"],
  },
  {
    slug: "export",
    group: "Data Platform",
  ...
  },
  {
    ...
    description: "Returns the health status of the API and its dependencies, including database connectivity and latency.",
    cacheTtl: "no-cache",
    parameters: [],
    sampleResponse: { status: "healthy", checks: [{ name: "database", status: "healthy", latency_ms: 5 }] },
    recipes: ["Monitor API uptime", "Health check for load balancers"],
  },
  {
    slug: "metrics",
    group: "Operations",
    ...
    method: "GET",
    path: "/metrics",
    summary: "Get API usage metrics and statistics.",
    description: "Returns request counts, average response times, error rates, and per-endpoint statistics from the last 24 hours.",
    cacheTtl: "1 minute",
    parameters: [],
    sampleResponse: { requests: { last_hour: 100, last_24_hours: 2400 }, endpoints: [] },
    recipes: ["Monitor API usage", "Track performance metrics"],
  },
  {
    slug: "events",
    group: "Operations",
    ...
    method: "GET",
    path: "/events",
    summary: "Server-Sent Events stream for data changes.",
    description: "Opens an SSE connection that streams enrichment status, recent API activity, and heartbeat events. Useful for real-time monitoring dashboards.",
    cacheTtl: "no-cache",
    parameters: [],
    sampleResponse: { event: "connected", message: "BDApi4All event stream connected" },
    recipes: ["Build real-time monitoring dashboards", "Track data enrichment progress"],
  },
  {
    slug: "fixture",
    group: "Developer Tools",
